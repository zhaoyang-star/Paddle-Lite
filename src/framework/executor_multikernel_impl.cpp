...
ExecutorMultiKernel<T>::ExecutorMultiKernel(
  ...
  for (int j = 0; j < ops.size(); ++j) {
    ...
    if (OpInfoMap<GPU_CL>::Instance()->Has(op_desc->Type())) {
      ...
      op_desc->SetComputeType(OpDesc::OpComputType::ComputeGPU);
      ops_of_block0_.push_back((OperatorBaseBase*)(op_handler_gpu.get()));
      ...
      op_desc->SetComputeType(OpDesc::OpComputType::ComputeCPU);

      ops_of_block0_.push_back((OperatorBaseBase*)(op_handler_cpu.get()));

    } else {
      PADDLE_MOBILE_ENFORCE(false, "unsupported op ( %s ) want to created",
                            op_desc->Type().c_str());
    }
...
#ifdef PADDLE_MOBILE_FPGA
template <typename T>
void ExecutorMultiKernel<T>::InjectVariable(const Tensor &t,
                                            std::string var_name) {
...
template <typename T>
void ExecutorMultiKernel<T>::FeedData(const Tensor &t) {
  InjectVariable(t, "feed0");
}

template <typename T>
void ExecutorMultiKernel<T>::FeedData(const std::vector<void *> &v) {
...
template <typename T>
void ExecutorMultiKernel<T>::GetResults(std::vector<void *> *v) {
...
template <typename T>
void ExecutorMultiKernel<T>::GetTensorResults(
...
template <typename T>
framework::Tensor *ExecutorMultiKernel<T>::GetTensorByName(
...
template <typename T>
std::shared_ptr<Tensor> ExecutorMultiKernel<T>::FetchResult(int id) {
...
template <typename T>
void ExecutorMultiKernel<T>::Predict_From_To(int start, int end) {
  auto &ops = ops_of_block0_;
...
template <typename T>
void ExecutorMultiKernel<T>::Predict_From(int start) {
  Predict_From_To(start);
}

template <typename T>
void ExecutorMultiKernel<T>::Predict_To(int end) {
