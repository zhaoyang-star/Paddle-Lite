...
#pragma mark - ExecutorCpu

template <typename T>
void ExecutorCpu<T>::SetThreadNum(int threads) {
  set_global_num_threads(threads);
}

template <typename T>
ExecutorCpu<T>::ExecutorCpu(const Program<T> &program,
    ...
    : program_(program),
      ...
      lod_mode_(lod_mode),
      config_(config) {
  DLOG << "ExecutorCpu in lod mode: " << lod_mode_;
...
template <typename T>
void ExecutorCpu<T>::InitFeedFetchList() {
...
template <typename T>
void ExecutorCpu<T>::InitCombineMemory() {
...
template <typename T>
void ExecutorCpu<T>::InitNoPersistableMemory(const Tensor &input_tensor) {
  for (const auto &block : program_desc_->Blocks()) {
    for (const auto &var_desc : block->Vars()) {
      auto var = program_.scope->Var(var_desc->Name());
      auto tensor = var->template GetMutable<LoDTensor>();
...
template <typename T>
bool ExecutorCpu<T>::varInputMemory(const std::shared_ptr<VarDesc> &var_desc,
                                    Variable *var) const {
#ifdef PADDLE_MOBILE_FPGA
...
template <typename T>
PMStatus ExecutorCpu<T>::Predict(
...
template <typename T>
std::vector<T> ExecutorCpu<T>::Predict(const std::vector<T> &input,
...
template <typename T>
void ExecutorCpu<T>::SetInput(const Tensor &input,
...
template <typename T>
void ExecutorCpu<T>::SetInput(const LoDTensor &input,
                              const std::string &var_name) {
  int index = 0;
  if (feed_indices_.find(var_name) != feed_indices_.end()) {
    index = feed_indices_.find(var_name)->second;
...
template <typename T>
std::shared_ptr<LoDTensor> ExecutorCpu<T>::GetOutput(
    const std::string &var_name) {
...
template class ExecutorCpu<float>;
