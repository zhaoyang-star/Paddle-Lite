...
void SequenceExpandKernelCpu<float>::Compute(const SequenceExpandParam &param) {
  const framework::LoDTensor *input_x = param.input_x_->LodTensor();
  const framework::LoDTensor *input_y = param.input_y_->LodTensor();
  framework::LoDTensor *output = param.output_->LodTensor();
