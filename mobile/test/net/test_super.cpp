  ...
  if (isok) {
    ...
    for (int i = 0; i < max; ++i) {
      ...
      std::cout << "640 * 360 predict cost :第" << i << ": "
                << paddle_mobile_lens::time_diff(time1, time2) << "ms" << std::endl;
    }
    ...
    auto time5 = paddle_mobile_lens::time();
    for (int i = 0; i < max; ++i) {
      auto time1 = paddle_mobile_lens::time();
    ...
    auto time6 = paddle_mobile_lens::time();
    std::cout << "720 * 480 predict cost :"
              << paddle_mobile_lens::time_diff(time5, time6) / max << "ms"
              << std::endl;
    ...
    auto time7 = paddle_mobile_lens::time();
    for (int i = 0; i < max; ++i) {
      auto time1 = paddle_mobile_lens::time();
    ...
    auto time8 = paddle_mobile_lens::time();
    std::cout << "1024 * 576 predict cost :"
              << paddle_mobile_lens::time_diff(time7, time8) / max << "ms"
              << std::endl;
    ...
    auto time9 = paddle_mobile_lens::time();
    for (int i = 0; i < max; ++i) {
      auto time1 = paddle_mobile_lens::time();
    ...
    auto time10 = paddle_mobile_lens::time();
    std::cout << "1280 * 720 predict cost :"
              << paddle_mobile_lens::time_diff(time9, time10) / max << "ms"
              << std::endl;
  }
